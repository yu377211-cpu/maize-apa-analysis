...
# 2. 读取输入文件（修正文件名）
# ============================
tau_data <- read.table("subpop_specific_Tau_results.txt", 
...
# 高特异性列表（全部组织）
high_list <- read.table("subpop_specific_allSubpop_specific_genes.txt", 
                        header = FALSE, stringsAsFactors = FALSE)[, 1]

# 保守列表
conserved_list <- read.table("subpop_specific_conserved_across_subgroups.txt", 
                             header = FALSE, stringsAsFactors = FALSE)[, 1]
...
# 5. 保存 PNG + PDF
# ============================
ggsave("Tau_distribution_final_subpop.png", 
       plot = p, width = 6, height = 5, dpi = 600, units = "in")

ggsave("Tau_distribution_final_subpop.pdf", 
