...
option_list <- list(
  make_option(c("-e", "--expr"), type="character", help="TMM CPM matrix, not log (row=PAC, col=sample)"),
...
# ===============================
# 5.5 Overall expression distribution (新增)
# ===============================
all_expr_vals <- as.vector(as.matrix(expr.log))

p_dist <- ggplot(data.frame(value = all_expr_vals), aes(x = value)) +
  geom_histogram(bins = 100, fill = "steelblue", color = "black") +
  theme_bw() +
  labs(
    title = "Overall expression distribution (log2 TMM CPM + 1)",
    x = "log2(CPM + 1)",
    y = "Count (PAC × sample)"
  )

ggsave(paste0(opt$out, "_dist.png"), p_dist, width = 6, height = 5)
ggsave(paste0(opt$out, "_dist.pdf"), p_dist, width = 6, height = 5)

cat("Overall distribution plot saved as", paste0(opt$out, "_dist.(png/pdf)"), "\n")
