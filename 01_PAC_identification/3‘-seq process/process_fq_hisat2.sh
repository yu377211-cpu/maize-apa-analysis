...
/share/apps/hisat2-2.1.0/hisat2 -p 4 -t --rna-strandness R -x /share2/pub/xingsl/xingsl/data_download/MZAPA/Ref/B73_V4/B73_V4 -U $outdir/$sample/$sample\.clean.AT.fq 2>$outdir/$sample/$sample\.maplog -S $outdir/$sample/$sample\.sam
#链特异性 如果使用dUTP: --rna-strandness R
/share/apps/samtools-1.9/bin/samtools view -F 20 $outdir/$sample/$sample\.sam |grep "NH:i:1" >$outdir/$sample/$sample\.F.sam
/share/apps/samtools-1.9/bin/samtools view -f 16 $outdir/$sample/$sample\.sam |grep "NH:i:1" >$outdir/$sample/$sample\.R.sam
cat $outdir/$sample/$sample\.F.sam $outdir/$sample/$sample\.R.sam >$outdir/$sample/$sample\.umap.sam
