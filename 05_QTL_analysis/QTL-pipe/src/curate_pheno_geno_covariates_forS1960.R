...
# remove individuals with more than 80% missing data (columns)
pdui_mat.sel <- pdui_mat.sel[, colMeans(is.na(pdui_mat.sel)) <= 0.8]

# filter PACs: NA < 0.5, Var > 0, IQR > 0
keep <- rowMeans(is.na(pdui_mat.sel)) < 0.5
pdui_mat.sel <- pdui_mat.sel[keep, ]
vars <- apply(pdui_mat.sel, 1, var, na.rm = TRUE)
keep <- vars > 0 & !is.na(vars)
pdui_mat.sel <- pdui_mat.sel[keep, ]
iqrs <- apply(pdui_mat.sel, 1, IQR, na.rm = TRUE)
keep <- iqrs > 0 & !is.na(iqrs)
pdui_mat.sel <- pdui_mat.sel[keep, ]
class(pdui_mat.sel) <- 'numeric'
...
str(mat.ds)
any(is.nan(mat.ds))
any(is.infinite(mat.ds))
summary(as.vector(mat.ds))
mat_impute <- impute.knn(mat.ds, maxp = 1000)
