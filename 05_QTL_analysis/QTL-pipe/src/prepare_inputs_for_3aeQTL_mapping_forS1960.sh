...
MAF="0.05"
TOP_N="5"
plink="/share/pub/xingsl/shilai/pipeline/tools/plink_1.9_20250819/plink"
#"/share/pub/chenfk/tools/plink"
vcftools="/share/apps/vcftools/bin/vcftools"
bin="/share/pub/xingsl/shilai/pipeline/tools/DaPars2/3aQTL-pipe"
...
function snp_and_3utr_location(){
	dapars2_res=$1
	python3 $bin/src/extract_SNP_location.py --genotype_bed ${currDir}/Matrix_eQTL/genotype_matrix.bed --output ${currDir}/Matrix_eQTL/snp_location.txt &
	wait
	#python3 $bin/src/extract_3UTR_location.py --dapars_res $dapars2_res --output ${currDir}/Matrix_eQTL/3UTR_location.txt &
	#perl /share/pub/xingsl/shilai/project/APA/3aQTL_link/3seq_PDUI_TPM/tiqu_3UTR.pl /share/pub/xingsl/shilai/project/APA/PAC_3seq_hq/region_dis/three_prime_UTR.merge_sort.bed2 $dapars2_res ${currDir}/Matrix_eQTL/3UTR_location.txt &
	perl /share/pub/xingsl/shilai/project/APA/AFreview/3aQTL/bin/tiqu_PAC_location.pl /share/pub/xingsl/shilai/project/APA/AFreview/PAC_3seq_hq/region_dis_addStrand/SRP115041_ab22.intersect.3UTR.location $dapars2_res ${currDir}/Matrix_eQTL/3UTR_location.txt &
	wait
...
function curate_pheno_geno_covariates(){
	...
	known_cov=$2
	topN=$3
	#/share/pub/xingsl/shilai/project/APA/software/miniforge3/envs/rpeer/bin/Rscript $bin/src/curate_pheno_geno_covariates.R -p $dapars2_res -c $known_cov -n $topN
	/share/pub/xingsl/shilai/project/APA/software/miniforge3/envs/R3.6.3/bin/Rscript $bin/src/curate_pheno_geno_covariates_forS1960_aeQTL.R -p $dapars2_res -c $known_cov -n $topN --cpm_threshold 5 --sample_pct 0.3
}
...
function PCA_on_genotype(){
	...
	$plink --bfile merged_plink --keep keep.list --geno 0.1 --maf $maf --biallelic-only --make-bed --out merged_plink_QC &
	wait
	sleep 20
	...
	wait
	$plink --bfile merged_plink_QC --extract merged_plink_QC.prune.in --pca 10 --out genotype_pca &
	wait
...
function generate_gt_matrix(){
	...
	do
		...
		sample=${filename%.*}
		python3 $bin/src/recode_with_012.py --frq $frq --GT ${currDir}/tmp/${sample}.GT.FORMAT --output ${currDir}/tmp/${sample}.GT.bed &
		wait
	done &
	wait
