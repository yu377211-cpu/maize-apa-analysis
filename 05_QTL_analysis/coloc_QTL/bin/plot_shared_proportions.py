...
def main():
    ...
    ax1.bar(x, a_pct, width, label="A-only", color=color_a_only)
    ax1.bar(x, shared_pct, width, bottom=a_pct, label='Shared (overlap)', color=color_shared)
    ax1.bar(x, b_pct, width, bottom=a_pct + shared_pct, label="B-only", color=color_b_only)

    # 添加百分比标签
    for i in range(len(tissues)):
        ax1.text(x[i], a_pct[i]/2, f'{a_pct[i]:.1f}%', ha='center', va='center', fontsize=9, color='white')
        y_shared_mid = a_pct[i] + shared_pct[i]/2
        ax1.text(x[i], y_shared_mid, f'{shared_pct[i]:.1f}%', ha='center', va='center', fontsize=9, color='white')
    ...
    ax1.set_ylabel('Percentage of total unique transcripts (%)', fontsize=12, fontweight='bold')
    ax1.set_ylim(0, 100)
    ax1.set_title('Composition of shared and unique transcripts', fontsize=14, fontweight='bold')
    ...
    ax2.set_ylabel('Shared transcript proportion (%)', fontsize=12, fontweight='bold')
